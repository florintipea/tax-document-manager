...
export interface ElsterSumCheck {
  id: string;
  fieldKey: string;
  labelDe: string;
  documentSum: number;
...
export interface ElsterValidationSummary {
  reviewCount: number;
  lowConfidenceCount: number;
  warnGapCount: number;
  sumChecks: ElsterSumCheck[];
  sumMismatchCount: number;
  duplicateDocumentIds: string[];
  /** Soft readiness for Mein-ELSTER checklist — does not block export */
...
export interface ElsterPreviewResult {
  ...
  checklist: string[];
  validation: ElsterValidationSummary;
}
...
const CATEGORY_TO_KIND: Array<{
  ...
  {
    match: /gehalt|lohnabrechnung|gehaltsabrechnung|w-2|pay.?stub|lohnsteuerbescheinigung/i,
    anlage: 'N',
    fieldKey: 'n.brutto',
  ...
  {
    match: /nebenkosten|hausgeld/i,
    anlage: 'V',
    fieldKey: 'v.nebenkosten',
    fieldLabelDe: 'Nebenkosten- / Hausgeldabrechnung (Bezug Objekt)',
    elsterHintDe: 'Je nach Rolle (Vermieter/Mieter) in Anlage V oder als Nachweis prüfen',
    needsReviewAlways: true,
  },
  {
    match: /mietvertrag|mietverträge|mietvertraege/i,
    anlage: 'V',
    fieldKey: 'v.mietvertrag',
    fieldLabelDe: 'Mietvertrag (Vermietung & Verpachtung)',
    elsterHintDe: 'Anlage V — Mietvertrag als Nachweis; Rolle und Beträge prüfen',
    needsReviewAlways: true,
  },
  {
  ...
  {
    match: /rechnung|quittung|belege?\b|invoice|receipt/i,
    anlage: 'N',
    fieldKey: 'n.werbungskosten',
    fieldLabelDe: 'Werbungskosten (Rechnung / Quittung — KI-Vorschlag)',
    elsterHintDe: 'Anlage N oder passende Anlage — Zuordnung prüfen (unverbindlich)',
    expense: true,
    needsReviewAlways: true,
  },
  {
  ...
  {
    match: /lohnausweis|auslandsbeleg|ausland.*lohn|foreign.?pay/i,
    anlage: 'AUS',
...
export function buildElsterPreview(input: {
  ...
  // Properties (Kauf / Mietvertrag-Vorschläge)
  for (const prop of properties) {
    const linked = linkDocs(parseDocumentIds(prop.documentIds), docById);
    const label = prop.label || prop.address;
    if (prop.monthlyRent != null && prop.monthlyRent > 0) {
      fields.push({
        anlage: 'V',
        fieldKey: `v.mietvertrag.miete.${prop.id}`,
        fieldLabelDe: `Kaltmiete (mtl.) — ${label}`,
        value: prop.monthlyRent,
        valueFormatted: formatEur(prop.monthlyRent),
        documents: linked,
        notes: [
          prop.roleHint ? `Rolle: ${prop.roleHint}` : null,
          prop.landlordName ? `Vermieter: ${prop.landlordName}` : null,
          prop.tenantName ? `Mieter: ${prop.tenantName}` : null,
          prop.contractStart ? `Beginn: ${prop.contractStart}` : null,
          prop.contractEnd ? `Ende: ${prop.contractEnd}` : null,
          'KI-Vorschlag aus Mietvertrag — bitte prüfen und bearbeiten',
        ]
          .filter(Boolean)
          .join(' · '),
        needsReview: prop.needsReview !== false,
        confidence: 'medium',
        source: 'mietvertrag',
        elsterHintDe: 'Anlage V — Mieteinnahmen (Jahreswert ggf. ×12) prüfen',
      });
    }
  ...
  // Nebenkosten- / Hausgeldabrechnungen for year
  for (const nk of nebenkosten.filter((n) => n.year === year)) {
    const linked = linkDocs(parseDocumentIds(nk.documentIds), docById);
    const kindLabel =
      nk.sourceKind === 'hausgeld' ? 'Hausgeldabrechnung' : 'Nebenkostenabrechnung';
    if (nk.umlagefaehigAmount != null && nk.umlagefaehigAmount > 0) {
      fields.push({
        anlage: 'V',
        fieldKey: `v.hausgeld.umlage.${nk.id}`,
        fieldLabelDe: `Umlagefähige Kosten — ${kindLabel}${nk.objectLabel ? ` — ${nk.objectLabel}` : ''}`,
        value: nk.umlagefaehigAmount,
        valueFormatted: formatEur(nk.umlagefaehigAmount),
        documents: linked,
        notes: nk.notes || 'KI-Vorschlag: Ausgaben V&V — bitte prüfen',
        needsReview: nk.needsReview !== false,
        confidence: 'medium',
        source: nk.sourceKind === 'hausgeld' ? 'hausgeld' : 'nebenkosten',
        elsterHintDe: 'Anlage V — Werbungskosten / Umlagen prüfen',
      });
    }
    if (nk.incomeAmount != null && nk.incomeAmount > 0) {
      fields.push({
        anlage: 'V',
        fieldKey: `v.hausgeld.einnahmen.${nk.id}`,
        fieldLabelDe: `Einnahmen / Gutschrift — ${kindLabel}${nk.objectLabel ? ` — ${nk.objectLabel}` : ''}`,
        value: nk.incomeAmount,
        valueFormatted: formatEur(nk.incomeAmount),
        documents: linked,
        notes: 'KI-Vorschlag: Einnahmen V&V — bitte prüfen',
        needsReview: true,
        confidence: 'medium',
        source: nk.sourceKind === 'hausgeld' ? 'hausgeld' : 'nebenkosten',
        elsterHintDe: 'Anlage V — Einnahmen prüfen',
      });
    }
    ...
    fields.push({
      anlage: 'V',
      fieldKey: `v.nk.${nk.id}`,
      fieldLabelDe: `${kindLabel} ${year}${nk.objectLabel ? ` — ${nk.objectLabel}` : ''}`,
      value: signed,
  ...
  // --- P0-1: Validation package (sum checks, duplicates, review queue) ---
  const sumChecks: ElsterSumCheck[] = [];
  for (const f of fields) {
    if (typeof f.value !== 'number' || !Number.isFinite(f.value)) continue;
    const docsWithAmount = f.documents.filter(
      (d) => d.taxAmount != null && Number.isFinite(d.taxAmount)
    );
    if (docsWithAmount.length === 0) continue;
    const documentSum =
      Math.round(docsWithAmount.reduce((s, d) => s + (d.taxAmount || 0), 0) * 100) / 100;
    const delta = Math.round(Math.abs(documentSum - f.value) * 100) / 100;
    const ok = delta <= 1;
    sumChecks.push({
      id: `sum-${f.fieldKey}`,
      fieldKey: f.fieldKey,
      labelDe: f.fieldLabelDe,
      documentSum,
      fieldValue: f.value,
      delta,
      ok,
    ...
    if (!ok) {
      gaps.push({
        id: `sum-mismatch-${f.fieldKey}`,
        severity: 'warn',
        messageDe: `Summen-Check: „${f.fieldLabelDe}“ (${formatEur(f.value)}) weicht von Belegsumme (${formatEur(documentSum)}) um ${formatEur(delta)} ab — bitte prüfen.`,
      });
      f.needsReview = true;
  ...
  // Duplicate documents: same name + same amount appearing more than once in year
  const dupFingerprint = new Map<string, string[]>();
  for (const d of yearDocs) {
    if (d.taxAmount == null) continue;
    const key = `${(d.name || '').toLowerCase().trim()}|${d.taxAmount}`;
    const list = dupFingerprint.get(key) || [];
    list.push(d.id);
    dupFingerprint.set(key, list);
  }
  const duplicateDocumentIds: string[] = [];
  for (const ids of dupFingerprint.values()) {
    if (ids.length > 1) {
      duplicateDocumentIds.push(...ids);
      gaps.push({
        id: `dup-docs-${ids[0]}`,
        severity: 'warn',
        messageDe: `Mögliche Doppelbelege erkannt (${ids.length}× gleicher Name/Betrag). Vor Übertragung in Mein ELSTER bereinigen.`,
        categoryHint: 'Dokumente',
      });
    }
  }

  // Same document linked to multiple conflicting numeric fields (info)
  const docFieldMap = new Map<string, string[]>();
  for (const f of fields) {
    if (typeof f.value !== 'number') continue;
    for (const d of f.documents) {
      const list = docFieldMap.get(d.id) || [];
      list.push(f.fieldKey);
      docFieldMap.set(d.id, list);
    }
  }
  for (const [docId, fieldKeys] of docFieldMap) {
    const unique = [...new Set(fieldKeys)];
    if (unique.length > 2) {
      gaps.push({
        id: `multi-map-${docId}`,
        severity: 'info',
        messageDe: `Ein Beleg ist mehreren Zeilen zugeordnet (${unique.length}). Zuordnung prüfen — Doppelzählung vermeiden.`,
      });
    }
  }

  if (!profile.steuerklasse && profile.country === 'DE') {
    gaps.push({
      id: 'pflicht-steuerklasse',
      severity: 'warn',
      messageDe: 'Pflichtfeld: Steuerklasse im Profil fehlt — vor Mein ELSTER ergänzen.',
      categoryHint: 'Profil',
    });
  }
  ...
  if (profile.country === 'DE') {
    const hasPersonName = Boolean(
      (profile.vorname && profile.vorname.trim()) ||
        (profile.nachname && profile.nachname.trim()) ||
        (profile.name && profile.name.trim())
    );
    if (!hasPersonName) {
      gaps.push({
        id: 'pflicht-profil-name',
        severity: 'warn',
        messageDe:
          'Steuerprofil unvollständig: Vor- und Nachname fehlen — unter Einstellungen ergänzen, bevor Sie in Mein ELSTER übertragen.',
        categoryHint: 'Profil',
      });
    }
    if (!profile.steuernummer && !profile.idNr) {
      gaps.push({
        id: 'pflicht-profil-steuernummer',
        severity: 'info',
        messageDe:
          'Steuernummer / IdNr fehlen im Profil (optional für die Vorschau, aber in Mein ELSTER nötig). Fiktive Testwerte nur intern verwenden.',
        categoryHint: 'Profil',
      });
    }
  }
  ...
  const lowConfidenceCount = fields.filter(
    (f) => f.confidence === 'low' || f.confidence === 'manual'
  ).length;
  const warnGapCount = gaps.filter((g) => g.severity === 'warn').length;
  const sumMismatchCount = sumChecks.filter((s) => !s.ok).length;

  const hasStructuredYearData =
    yearDocs.length > 0 ||
    taxLines.some((l) => l.year === year) ||
    rentals.some((r) => r.year === year) ||
    nebenkosten.some((n) => n.year === year) ||
    Boolean(gg && gg.enabled && (gg.foreignEmploymentIncome > 0 || gg.foreignWithholdingTax > 0));

  const blockersDe: string[] = [];
  if (!hasStructuredYearData && fields.length === 0) {
    blockersDe.push(
      'Keine Belege und keine Zeilen für dieses Jahr — zuerst Dokumente hochladen oder Beträge erfassen.'
    );
  }
  if (warnGapCount > 0) {
    blockersDe.push(`${warnGapCount} Warnung(en) in der Validierung — bitte prüfen-Queue öffnen.`);
  }
  if (reviewCount > 0) {
    blockersDe.push(`${reviewCount} Zeile(n) mit „prüfen“-Markierung.`);
  }

  // Soft readiness: structured lines/GG/rental count even without uploaded Belege.
  const readyForChecklist =
    hasStructuredYearData &&
    fields.length > 0 &&
    warnGapCount === 0 &&
    sumMismatchCount === 0;

  if (!readyForChecklist) {
    checklist.unshift(
      'Validierung: Warnungen und „prüfen“-Zeilen in TaxDoc abarbeiten, bevor Sie Werte in Mein ELSTER übertragen.'
    );
  }

  const validation: ElsterValidationSummary = {
    reviewCount,
    lowConfidenceCount,
    warnGapCount,
    sumChecks,
    sumMismatchCount,
    duplicateDocumentIds: [...new Set(duplicateDocumentIds)],
    readyForChecklist,
    blockersDe,
  };
