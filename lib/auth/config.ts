  ...
  providers: [
    Credentials({
      ...
      async authorize(credentials) {
        ...
        // Magic link login (zero-friction save)
        if (credentials?.magicToken && credentials?.email) {
          const magicValidated = magicLoginSchema.safeParse({
            email: credentials.email,
            magicToken: credentials.magicToken,
          });
          if (!magicValidated.success) {
            throw new Error("Invalid magic link");
          }
          const { consumeMagicLinkToken, ensureUserForMagicLink } = await import(
            "@/lib/auth/magic-link"
          );
          const ok = await consumeMagicLinkToken(
            magicValidated.data.email,
            magicValidated.data.magicToken
          );
          if (!ok) {
            throw new Error("Magic link expired or invalid");
          }
          const user = await ensureUserForMagicLink(magicValidated.data.email);
        ...
        // Allowlisted admin emails always get admin role (resilient if DB role drifted)
        ...
        if (
          ...
          effectiveRole = "admin";
          console.log(`Promoted allowlisted admin email to role=admin: ${user.email}`);
        }
