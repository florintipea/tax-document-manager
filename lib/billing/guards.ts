...
> {
  ...
  if (features.aiMessagesPerMonth === 0) {
    return {
      ...
      response: tierError(
        'AI_NOT_INCLUDED',
        'AI assistant is not included in your plan. Upgrade to Standard.',
        'standard'
      ),
    };
  }
  ...
  if (used >= features.aiMessagesPerMonth) {
    return {
      ...
      response: tierError(
        'AI_LIMIT',
        `Monthly AI message limit reached (${features.aiMessagesPerMonth}). Upgrade to Standard for unlimited AI.`,
        'standard'
      ),
    };
  }
