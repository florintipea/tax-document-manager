...
/** Cap in-memory keys so unique-IP traffic cannot grow the heap unbounded on Starter. */
const MEMORY_STORE_MAX_KEYS = 5_000;
const memoryStore = new TtlMap<{ count: number }>({
  maxSize: MEMORY_STORE_MAX_KEYS,
  // Window length varies per preset; entries carry their own expiresAt via set(ttl).
  ttlMs: 60 * 60 * 1000,
});

/** Test/ops helper — current in-memory key count. */
export function getMemoryRateLimitSize(): number {
...
async function getRedis(): Promise<Redis | null> {
  ...
  if (!redis) {
    try {
      const { Redis: RedisClient } = await import(/* webpackIgnore: true */ 'ioredis');
...
function checkMemoryRateLimit(
  ...
  const now = Date.now();
  const existing = memoryStore.peek(key);
  const count = existing?.value.count ?? 0;

  if (count >= options.maxRequests) {
    return {
      allowed: false,
      remaining: 0,
      resetTime: new Date(existing?.expiresAt ?? now + options.windowMs),
    };
  }

  const nextCount = count + 1;
  // First hit opens the window; later hits preserve remaining TTL.
  const ttlForSet = existing
    ? Math.max(1, existing.expiresAt - now)
    : options.windowMs;

  memoryStore.set(key, { count: nextCount }, ttlForSet);

  return {
    allowed: true,
    remaining: Math.max(0, options.maxRequests - nextCount),
    resetTime: new Date(now + ttlForSet),
  };
}
