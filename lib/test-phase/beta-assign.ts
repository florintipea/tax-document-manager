...
import { ensureWelcomeSupportThread } from '@/lib/support/threads';
...
export async function assignNextBetaSlot(
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      ...
      // Best-effort: welcome chat + admin ping (outside txn)
      try {
        await ensureWelcomeSupportThread({
          userId: result.userId,
          guestEmail: email,
          testerEmail: result.testerEmail,
          loginUrl: result.loginUrl,
          assignedToName: name,
        });
      } catch (err) {
        console.error('Welcome support thread failed:', err);
      }
      ...
      try {
        await createAdminNotification({
          ...
          body: `${name || email} hat einen Beta-Zugang erhalten (${result.testerEmail}).`,
          link: '/admin/support',
          metadata: {
            assignedToEmail: email,
            testerEmail: result.testerEmail,
        ...
        console.error('Beta request notification failed:', err);
      }
