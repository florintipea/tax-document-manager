...
async function main() {
  ...
  // Docker runs as non-root `nextjs` — /app/dist is not writable. Skip on Render.
  if (process.env.SKIP_TEST_ACCOUNT_FILES === 'true') {
    console.log(`\n🎉 ${ACCOUNT_COUNT} test accounts ready (file export skipped).`);
  ...
  try {
    ...
    console.warn(
      `Skipping test-account file export (${err.code || err.message}). Accounts were still created in the DB.`
    );
  }
