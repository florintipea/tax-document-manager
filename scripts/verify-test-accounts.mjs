...
function storeCookies(response) {
  ...
  if (typeof response.headers.getSetCookie === 'function') {
    ...
    if (setCookie) {
      // Fallback for runtimes that expose a single header string.
...
async function main() {
  ...
  console.log(`Expected tester format count: ${ACCOUNT_COUNT}`);

  const samples = [
    formatTesterEmail(1, DOMAIN, ACCOUNT_COUNT),
    formatTesterEmail(Math.min(25, ACCOUNT_COUNT), DOMAIN, ACCOUNT_COUNT),
    formatTesterEmail(Math.min(50, ACCOUNT_COUNT), DOMAIN, ACCOUNT_COUNT),
    formatTesterEmail(Math.min(ACCOUNT_COUNT, MAX_TESTER_COUNT), DOMAIN, ACCOUNT_COUNT),
  ];
