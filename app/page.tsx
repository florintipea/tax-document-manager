"use client";

import { useRouter } from "next/navigation";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { Calculator, FileText, Bot, Shield, Sparkles } from "lucide-react";
import { AppLogo } from "@/components/brand/app-logo";
import { CloudInstanceBanner } from "@/components/cloud/cloud-instance-banner";
import { AppFooter } from "@/components/layout/footer";
import { QuickCheck } from "@/components/growth/quick-check";
import { ReferralInviteCard } from "@/components/growth/referral-invite-card";
import { useI18n } from "@/lib/i18n/provider";

export default function Home() {
  const router = useRouter();
  const { t } = useI18n();

  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-slate-50 via-blue-50 to-sky-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      <div className="container mx-auto px-4 py-12 sm:py-16 flex-1">
        <div className="max-w-3xl mx-auto mb-8">
          <CloudInstanceBanner />
        </div>
        <div className="text-center">
          <div className="flex justify-center mb-6">
            <AppLogo size="xl" textClassName="bg-clip-text text-transparent bg-gradient-to-r from-[#1A3FA8] to-[#2B5FCC]" />
          </div>

          <div className="inline-flex flex-wrap items-center justify-center gap-2 mb-6">
            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-100 dark:bg-blue-900/30 rounded-full text-sm font-medium text-blue-700 dark:text-blue-300">
              <Sparkles className="w-3.5 h-3.5" />
              {t("landing.badge")}
            </span>
            <span className="inline-flex px-3 py-1.5 bg-emerald-50 dark:bg-emerald-900/30 rounded-full text-xs font-medium text-emerald-800 dark:text-emerald-200 border border-emerald-200 dark:border-emerald-800">
              {t("landing.trustNoAdvice")}
            </span>
            <span className="inline-flex px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-full text-xs font-medium text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-600">
              {t("landing.trustNoAutoSubmit")}
            </span>
          </div>

          <h1 className="text-3xl md:text-5xl text-gray-900 dark:text-white mb-4 font-bold max-w-3xl mx-auto">
            {t("landing.headline")}
          </h1>

          <p className="text-lg md:text-xl text-gray-600 dark:text-gray-400 mb-3 max-w-2xl mx-auto">
            {t("landing.subheadline")}
          </p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 max-w-2xl mx-auto">
            {t("landing.yearRoundValue")}
          </p>

          <p className="text-sm text-amber-800 dark:text-amber-200 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800 rounded-lg px-4 py-3 max-w-2xl mx-auto mb-6">
            {t("landing.disclaimer")}
          </p>

          <div className="max-w-2xl mx-auto mb-8 rounded-xl border border-blue-200 bg-white/80 px-4 py-4 text-left dark:border-blue-800 dark:bg-gray-800/80">
            <p className="font-semibold text-gray-900 dark:text-white">
              {t("landing.pricingFairTitle")}
            </p>
            <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
              {t("landing.pricingFairBody")}
            </p>
            <div className="mt-3 flex flex-wrap gap-3 text-sm">
              <Link href="/pricing" className="font-medium text-blue-600 hover:underline dark:text-blue-400">
                {t("pricing.nav")}
              </Link>
              <Link href="/trust" className="font-medium text-blue-600 hover:underline dark:text-blue-400">
                {t("trust.nav")}
              </Link>
              <Link href="/rechner" className="font-medium text-blue-600 hover:underline dark:text-blue-400">
                {t("growthNav.rechner")}
              </Link>
            </div>
          </div>

          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-10">
            <Button
              variant="primary"
              size="lg"
              className="shadow-lg hover:shadow-xl transition-shadow"
              onClick={() => router.push("/rechner")}
            >
              {t("landing.ctaEstimate")}
            </Button>
            <Button
              variant="outline"
              size="lg"
              onClick={() => router.push("/beleg-check")}
            >
              {t("landing.ctaChecklist")}
            </Button>
          </div>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-10">
            <button
              type="button"
              className="underline hover:text-blue-600 dark:hover:text-blue-400"
              onClick={() => router.push("/auth/login")}
            >
              {t("landing.signIn")}
            </button>
            {" · "}
            <button
              type="button"
              className="underline hover:text-blue-600 dark:hover:text-blue-400"
              onClick={() => router.push("/auth/register")}
            >
              {t("landing.getStarted")}
            </button>
            {" · "}
            <button
              type="button"
              className="underline hover:text-blue-600 dark:hover:text-blue-400"
              onClick={() => router.push("/beta-anfrage")}
            >
              {t("landing.betaCta")}
            </button>
          </p>
        </div>

        <div className="mx-auto mb-12 max-w-xl">
          <QuickCheck showPostCta />
        </div>

        <div className="mx-auto mb-12 max-w-xl">
          <ReferralInviteCard />
        </div>

        <div className="mt-8 grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-5xl mx-auto">
          <div className="group bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
            <Calculator className="w-8 h-8 text-purple-600 dark:text-purple-400 mb-3" />
            <h3 className="text-lg font-semibold mb-2 text-gray-900 dark:text-white">
              {t("landing.featureCalculatorTitle")}
            </h3>
            <p className="text-sm text-gray-600 dark:text-gray-300">
              {t("landing.featureCalculatorDesc")}
            </p>
          </div>

          <div className="group bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
            <FileText className="w-8 h-8 text-blue-600 dark:text-blue-400 mb-3" />
            <h3 className="text-lg font-semibold mb-2 text-gray-900 dark:text-white">
              {t("landing.featureDocumentsTitle")}
            </h3>
            <p className="text-sm text-gray-600 dark:text-gray-300">
              {t("landing.featureDocumentsDesc")}
            </p>
          </div>

          <div className="group bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
            <Bot className="w-8 h-8 text-green-600 dark:text-green-400 mb-3" />
            <h3 className="text-lg font-semibold mb-2 text-gray-900 dark:text-white">
              {t("landing.featureAITitle")}
            </h3>
            <p className="text-sm text-gray-600 dark:text-gray-300">
              {t("landing.featureAIDesc")}
            </p>
          </div>

          <div className="group bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700">
            <Shield className="w-8 h-8 text-emerald-600 dark:text-emerald-400 mb-3" />
            <h3 className="text-lg font-semibold mb-2 text-gray-900 dark:text-white">
              {t("landing.featureSecurityTitle")}
            </h3>
            <p className="text-sm text-gray-600 dark:text-gray-300">
              {t("landing.featureSecurityDesc")}
            </p>
          </div>
        </div>

        <div className="mt-10 text-center max-w-2xl mx-auto">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            {t("landing.otherCountriesNote")}
          </p>
          <p className="mt-3 text-sm text-gray-600 dark:text-gray-400">
            {t("landing.grenzgaengerTeaser")}{" "}
            <Link href="/grenzgaenger" className="text-blue-600 hover:underline dark:text-blue-400">
              {t("growthNav.grenzgaenger")}
            </Link>
            {" · "}
            <Link href="/beleg-check" className="text-blue-600 hover:underline dark:text-blue-400">
              {t("landing.featureDocumentsTitle")}
            </Link>
          </p>
        </div>

        <div className="mt-6 text-center text-sm text-gray-500 dark:text-gray-400">
          <Link href="/pricing" className="hover:underline mx-2">
            {t("pricing.nav")}
          </Link>
          <span>·</span>
          <Link href="/trust" className="hover:underline mx-2">
            {t("trust.nav")}
          </Link>
          <span>·</span>
          <Link href="/legal/impressum" className="hover:underline mx-2">
            {t("legal.impressum")}
          </Link>
          <span>·</span>
          <Link href="/legal/datenschutz" className="hover:underline mx-2">
            {t("legal.datenschutz")}
          </Link>
        </div>
      </div>
      <AppFooter />
    </div>
  );
}
