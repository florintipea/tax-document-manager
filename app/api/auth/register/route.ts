...
export async function POST(request: NextRequest) {
  try {
    ...
    const { email, password, name, country, language, ref } = validated.data;
    ...
    // Growth Phase 1: attribute referral (non-blocking)
    try {
      const { applyReferralOnRegister } = await import("@/lib/growth/referral");
      await applyReferralOnRegister(user.id, ref);
    } catch (refError) {
