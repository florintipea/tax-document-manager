...
import {
  ...
  tryDeliverOnboardingEmail,
} from '@/lib/test-phase/beta-onboarding';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const emailDelivery = await tryDeliverOnboardingEmail({
    ...
    return NextResponse.json(
      {
        ...
        usageSteps: [
          ...
          'Dokument hochladen und pruefen',
          'Steuerrechner oder ELSTER-Assistent testen',
          'Feedback in der App senden',
        ],
        privacyNote:
          'Wir nutzen deine Daten nur fuer die Beta-Betreuung. Teile die Zugangsdaten nicht mit Dritten.',
        packetText: buildOnboardingPacket(result),
        emailDelivery,
      },
      { status: 201 }
    );
