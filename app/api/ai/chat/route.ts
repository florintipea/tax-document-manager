...
export async function POST(request: NextRequest) {
  try {
    ...
    const documentCount = await db.document.count({ where: { userId } });

    const recentDocuments = (user?.documents ?? []).map((doc) => ({
      name: doc.name,
      originalName: doc.originalName,
      year: doc.year,
      taxCategory: doc.taxCategory,
      categoryName: doc.category?.name ?? null,
      taxAmount: doc.taxAmount,
      isTaxRelevant: doc.isTaxRelevant,
    }));
    ...
    const context = {
      ...
      isCrossBorder: user?.isCrossBorder,
    ...
    if (!configured) {
      return NextResponse.json(
        {
          error:
            'Kein KI-Anbieter verbunden. Bitte unter Einstellungen → KI-Anbieter Ihren eigenen API-Key hinterlegen.',
    ...
    const detail = sanitizeAiProviderError(error);
    return NextResponse.json(
      {
        error: detail
...
function sanitizeAiProviderError(error: unknown): string | null {
  if (!(error instanceof Error) || !error.message?.trim()) return null;
  return error.message
    .replace(/sk-[a-zA-Z0-9._-]{8,}/g, "[key]")
    .replace(/sk-ant-[a-zA-Z0-9._-]{8,}/g, "[key]")
