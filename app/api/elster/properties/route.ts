...
const schema = z.object({
  ...
  roleHint: z.enum(['vermieter', 'mieter', 'unknown']).optional().nullable(),
  monthlyRent: z.number().min(0).optional().nullable(),
  contractStart: z.string().max(32).optional().nullable(),
  contractEnd: z.string().max(32).optional().nullable(),
  landlordName: z.string().max(200).optional().nullable(),
  tenantName: z.string().max(200).optional().nullable(),
  needsReview: z.boolean().optional(),
  sourceHint: z.string().max(64).optional().nullable(),
});
  ...
  try {
    ...
    const property = await db.property.create({
      data: {
        ...
        sourceHint: data.sourceHint ?? 'manual',
      },
    });
    ...
    const property = await db.property.update({
      ...
      data: {
        ...
        ...(data.documentIds !== undefined
          ? { documentIds: serializeDocumentIds(data.documentIds) }
          : {}),
        ...(data.roleHint !== undefined ? { roleHint: data.roleHint } : {}),
        ...(data.monthlyRent !== undefined ? { monthlyRent: data.monthlyRent } : {}),
        ...(data.contractStart !== undefined ? { contractStart: data.contractStart } : {}),
        ...(data.contractEnd !== undefined ? { contractEnd: data.contractEnd } : {}),
        ...(data.landlordName !== undefined ? { landlordName: data.landlordName } : {}),
        ...(data.tenantName !== undefined ? { tenantName: data.tenantName } : {}),
        ...(data.needsReview !== undefined ? { needsReview: data.needsReview } : {}),
        ...(data.sourceHint !== undefined ? { sourceHint: data.sourceHint } : {}),
      },
    });
