...
const schema = z.object({
  ...
  umlagefaehigAmount: z.number().min(0).optional().nullable(),
  incomeAmount: z.number().min(0).optional().nullable(),
  sourceKind: z.enum(['hausgeld', 'nebenkosten', 'other']).optional().nullable(),
  ...
  try {
    ...
    const row = await db.nebenkostenAbrechnung.create({
      data: {
        ...
        umlagefaehigAmount: data.umlagefaehigAmount ?? null,
