...
export default function DocumentsPage() {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (files: File[]) => {
      const CHUNK = uploadLimits.uploadChunkSize;
      const allDocs: Document[] = [];
      const skippedFiles: string[] = [];
      const contentDuplicates: DuplicateUploadItem[] = [];
      // Keine künstliche Datei-Obergrenze — große Auswahl in Paketen hochladen.
      ...
      for (let i = 0; i < fileList.length; i += CHUNK) {
        ...
        if (Array.isArray(data.skippedFiles)) skippedFiles.push(...data.skippedFiles);
        if (Array.isArray(data.contentDuplicates)) {
          contentDuplicates.push(...data.contentDuplicates);
        }
      }

      if (allDocs.length === 0) {
        throw new Error(
          skippedFiles[0] || "Upload fehlgeschlagen"
        );
      }
