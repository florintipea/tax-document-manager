...
import { Calculator, Receipt, AlertTriangle, Lightbulb, TrendingDown, RefreshCw } from "lucide-react";
...
function CalculatorPageInner() {
  ...
  const initialCountryRef = useRef(true);
  ...
  useEffect(() => {
    ...
    // On first load, create defaults then overlay any pending draft.
    // On later country changes, reset deductions for the new country.
    const defaults = createDefaultDeductions(country);
    let next = defaults.map((d) =>
    ...
    if (initialCountryRef.current && draft && Array.isArray(draft.deductions)) {
      const draftDeductions = draft.deductions as DeductionInput[];
      if (draftDeductions.length > 0) {
        next = draftDeductions;
      }
    }
    if (initialCountryRef.current) {
      initialCountryRef.current = false;
  ...
  const persistCalculatorDraft = useCallback(() => {
    if (draftSkipRef.current) return;
    ...
    draftSyncRef.current = setTimeout(() => {
      ...
      void fetch("/api/user/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ calculatorDraft: JSON.stringify(draft) }),
      }).then(async (response) => {
        if (!response.ok) {
          const data = await response.json().catch(() => ({}));
          // Non-blocking: column may be missing until migrate; avoid noisy toasts on every keystroke
  ...
  }, [draftHydrated, persistCalculatorDraft]);
  ...
  useEffect(() => {
    ...
    const loadSettings = async () => {
      try {
        ...
        if (typeof s?.profileRefreshNotice === "string" && s.profileRefreshNotice) {
          setProfileRefreshNotice(s.profileRefreshNotice);
        }

        // Restore Steuerrechner amounts from permanent draft (deductions applied after defaults)
        if (typeof s?.calculatorDraft === "string" && s.calculatorDraft.trim()) {
          try {
            const draft = JSON.parse(s.calculatorDraft) as Record<string, unknown>;
            pendingDraftRef.current = draft;
            if (typeof draft.income === "string") setIncome(draft.income);
            if (typeof draft.taxWithheld === "string") setTaxWithheld(draft.taxWithheld);
            if (typeof draft.filingStatus === "string") {
              setFilingStatus(draft.filingStatus as FilingStatus);
            }
            if (typeof draft.caProvince === "string") {
              setCaProvince(draft.caProvince as CAProvinceCode);
            }
            if (typeof draft.spouseTaxWithheld === "string") {
              setSpouseTaxWithheld(draft.spouseTaxWithheld);
            }
            if (typeof draft.spouseWerbungskosten === "string") {
              setSpouseWerbungskosten(draft.spouseWerbungskosten);
            }
            if (draft.vorauszahlungen && typeof draft.vorauszahlungen === "object") {
              setVorauszahlungen({
                ...emptyVorauszahlungen,
                ...(draft.vorauszahlungen as VorauszahlungInput),
              });
            }
            if (typeof draft.showVorauszahlungen === "boolean") {
              setShowVorauszahlungen(draft.showVorauszahlungen);
            }
            if (draft.crossBorder && typeof draft.crossBorder === "object") {
              setCrossBorder((prev) => ({
                ...prev,
                ...(draft.crossBorder as CrossBorderInput),
              }));
            }
            if (draft.rental && typeof draft.rental === "object") {
              setRental((prev) => ({
                ...prev,
                ...(draft.rental as RentalIncomeInput),
              }));
            }
            const meta =
              draft.autofillMeta && typeof draft.autofillMeta === "object"
                ? (draft.autofillMeta as Record<string, unknown>)
                : null;
            if (meta) {
              const conf =
                meta.confidenceLabel === "high"
                  ? "Konfidenz: hoch"
                  : meta.confidenceLabel === "medium"
                    ? "Konfidenz: mittel"
              ...
              setAutofillNotice(
                [
                  typeof meta.disclaimerDe === "string" ? meta.disclaimerDe : null,
                  typeof meta.sourceSummaryDe === "string"
                    ? meta.sourceSummaryDe
                    : null,
                  conf,
                  "bitte prüfen",
                ]
        ...
        // Allow draft persistence after hydration settles
        setTimeout(() => {
  ...
  const applyAutofillDraft = useCallback((draft: Record<string, unknown>) => {
    if (typeof draft.income === "string") setIncome(draft.income);
    if (typeof draft.taxWithheld === "string") setTaxWithheld(draft.taxWithheld);
    if (typeof draft.steuerklasse === "string") {
      setSteuerklasse(draft.steuerklasse as Steuerklasse);
    }
    if (draft.deFilingMode === "zusammen" || draft.deFilingMode === "einzel") {
      setDeFilingMode(draft.deFilingMode);
    }
    if (typeof draft.spouseIncome === "string") setSpouseIncome(draft.spouseIncome);
    if (Array.isArray(draft.deductions)) {
      setDeductions(draft.deductions as DeductionInput[]);
    }
    if (draft.rental && typeof draft.rental === "object") {
      setRental((prev) => ({ ...prev, ...(draft.rental as RentalIncomeInput) }));
    }
    if (draft.crossBorder && typeof draft.crossBorder === "object") {
      setCrossBorder((prev) => ({
        ...prev,
    ...
    const summary =
      typeof draft.sourceSummaryDe === "string" ? draft.sourceSummaryDe : null;
    const disclaimer =
      typeof draft.disclaimerDe === "string" ? draft.disclaimerDe : null;
    const confidence =
      draft.confidenceLabel === "high"
        ? "Konfidenz: hoch"
        : draft.confidenceLabel === "medium"
          ? "Konfidenz: mittel"
          : draft.confidenceLabel === "low"
            ? "Konfidenz: niedrig"
            : null;
    setAutofillNotice(
      [disclaimer, summary, confidence, "bitte prüfen"]
        .filter(Boolean)
        .join(" — ") || "Belege übernommen — bitte prüfen"
    );
  }, []);

  const runAutofillFromBelege = useCallback(
    async (runBatchFirst = false) => {
      if (autofillBusy) return;
      setAutofillBusy(true);
      try {
        const yearParam = searchParams.get("year");
        const year = yearParam ? parseInt(yearParam, 10) : new Date().getFullYear();
        const res = await fetch("/api/tax/autofill-from-belege", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ year, runBatchFirst, persist: true }),
        });
        const data = await res.json().catch(() => ({}));
        if (!res.ok) {
          throw new Error(
            typeof data.error === "string" ? data.error : "Autofill fehlgeschlagen"
          );
        }
        if (data.draft && typeof data.draft === "object") {
          applyAutofillDraft(data.draft as Record<string, unknown>);
        }
        toast.success("Steuerrechner aus Belegen/Profil gefüllt — bitte prüfen");
      } catch (err) {
        toast.error(err instanceof Error ? err.message : "Autofill fehlgeschlagen");
      } finally {
        setAutofillBusy(false);
      }
    },
    [applyAutofillDraft, autofillBusy, searchParams]
  );

  useEffect(() => {
    if (!draftHydrated || autofillTriggered.current) return;
    if (searchParams.get("autofill") === "1") {
      autofillTriggered.current = true;
      void runAutofillFromBelege(false);
    }
  }, [draftHydrated, runAutofillFromBelege, searchParams]);
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-2xl mx-auto">
            ...
            {autofillNotice && (
              <div className="mb-4 rounded-lg border border-blue-300 bg-blue-50 px-4 py-3 text-sm text-blue-950 dark:border-blue-800 dark:bg-blue-950/30 dark:text-blue-100">
                {autofillNotice}
              </div>
            )}
            ...
            <div className="mb-4 flex flex-wrap gap-2">
              <Button
                ...
                disabled={autofillBusy || !isDE}
                onClick={() => void runAutofillFromBelege(false)}
              >
                Aus Belegen/Profil übernehmen
              </Button>
              <Button
                type="button"
                size="sm"
                variant="outline"
                disabled={autofillBusy || !isDE}
                onClick={() => void runAutofillFromBelege(true)}
              >
                Belege neu zuordnen + übernehmen
              </Button>
            </div>
