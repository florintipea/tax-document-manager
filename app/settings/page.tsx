...
import { Settings, User, Shield, Bell, Moon, Sun, Monitor, CreditCard, Sparkles, Receipt, Scale } from "lucide-react";
...
import { PrivacyDataControls } from "@/components/settings/privacy-data-controls";
import {
  EMPTY_TAX_PROFILE,
  SteuerprofilForm,
...
function SettingsPageContent() {
  ...
  const [activeTab, setActiveTab] = useState(
    tabFromUrl && tabFromUrl.length > 0 ? tabFromUrl : "profile"
  );
  ...
  const [profileRefreshNotice, setProfileRefreshNotice] = useState<string | null>(
    null
  );
    ...
    const loadSettings = async () => {
      try {
        ...
        setProfileRefreshNotice(
          typeof data.settings.profileRefreshNotice === "string"
            ? data.settings.profileRefreshNotice
            : null
        );
  ...
  const applyTaxProfileSettings = (settings: Record<string, unknown>) => {
    ...
    if (typeof settings.profileRefreshNotice === "string") {
      setProfileRefreshNotice(settings.profileRefreshNotice);
    } else if (settings.profileRefreshNotice === null) {
      setProfileRefreshNotice(null);
    }
  };

  const dismissProfileRefreshNotice = useCallback(async () => {
    setProfileRefreshNotice(null);
    try {
      await fetch("/api/user/settings", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ profileRefreshNotice: null }),
      });
    } catch {
      /* non-fatal */
    }
  ...
  const tabs = [
    ...
    { id: "privacy", label: t("settings.privacy"), icon: Scale },
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
              ...
              <div className="p-6">
                ...
                {activeTab === "taxProfile" && (
                  <>
                    ...
                    {profileRefreshNotice && (
                      <div className="mb-4 flex flex-wrap items-start justify-between gap-2 rounded-lg border border-teal-400 bg-teal-50 px-4 py-3 text-sm font-medium text-teal-950 dark:border-teal-700 dark:bg-teal-950/40 dark:text-teal-100">
                        <span>{profileRefreshNotice}</span>
                        <Button
                          type="button"
                          size="sm"
                          variant="outline"
                          onClick={() => void dismissProfileRefreshNotice()}
                        >
                          Hinweis schließen
                        </Button>
                      </div>
                    ...
                    <div className="mb-6">
                      <PriorElsterUploadPanel
                        ...
                        belegeYear={new Date().getFullYear()}
                        onProfileUpdated={async () => {
                          try {
                            const response = await fetch("/api/user/settings");
                            if (!response.ok) return;
                            const data = await response.json();
                            applyTaxProfileSettings(data.settings || {});
                          } catch {
                ...
                {activeTab === "privacy" && <PrivacyDataControls />}
