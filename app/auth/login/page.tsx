...
import { getLoginUiMessage } from "@/lib/auth/login-errors";
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!checkResponse.ok) {
        const code = checkData.code as string | undefined;
        if (
          code === "ACCOUNT_LOCKED" ||
          code === "RATE_LIMITED" ||
          checkResponse.status === 423 ||
          checkResponse.status === 429
        ) {
          toast.error(
            checkData.error ||
              getLoginUiMessage(checkResponse.status, checkData, t)
          );
        } else if (
        ...
        } else if (code === "ACCESS_DENIED" || checkResponse.status === 403) {
          toast.error(
            checkData.error || t("auth.accessDenied")
          );
        } else if (code === "SERVER_ERROR" || checkResponse.status >= 500) {
          toast.error(t("auth.serverError"));
        } else {
          toast.error(getLoginUiMessage(checkResponse.status, checkData, t));
        }
        setIsLoading(false);
        return;
      }
